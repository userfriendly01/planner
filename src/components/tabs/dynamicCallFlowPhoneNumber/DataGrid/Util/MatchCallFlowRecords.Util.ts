 ...
 */
export function hasDuplicateCallFlowRecord(searchList: Array<PhoneNumberRecordType>, matchCandidate: PhoneNumberRecordType, alertBarState: AlertBarState, matchFilter: MatchFilter = pkeyFilter): boolean {
  return hasDuplicateCallFlowRecords(searchList, [matchCandidate], alertBarState, matchFilter);
}
 ...
 */
export function hasDuplicateCallFlowRecords(searchList: Array<PhoneNumberRecordType>, matchCandidateList: Array<PhoneNumberRecordType>,  alertBarState: AlertBarState, matchFilter: MatchFilter = pkeyFilter): boolean {
...
function generateMatchingRecordMessages(searchList: Array<PhoneNumberRecordType>, matchCandidateList: Array<PhoneNumberRecordType>, matchFilter: MatchFilter): Array<string> {
  return findMatchingCallFlowRecords(searchList, matchCandidateList, matchFilter).map((matchCandidateRecord: PhoneNumberRecordType) =>
 ...
 * @param {Array<PhoneNumberRecordType>} matchCandidateList - If passing single phoneNumberRecord, pass it as single length array i.e. "[newCallFlowRecord]"
 * @param {MatchFilter} matchFilter - Filter to use to match records
 *
 * @return {Array<PhoneNumberRecordType>} - List of matching phone number records
 */
export function findMatchingCallFlowRecords(searchList: Array<PhoneNumberRecordType>, matchCandidateList: Array<PhoneNumberRecordType>, matchFilter: MatchFilter): Array<PhoneNumberRecordType> {
