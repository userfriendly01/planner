...
import { PhoneNumberFormFieldConfigs } from "../Field/LegacyPhoneNumberFieldsConfig";
...
import { ToggleAddOrViewListIcon } from "./ToggleAddOrViewListIcon";
...
export const AddDynamicPhoneNumber = ({
  ...
  useEffect(() => {
    ...
    formFieldViewListIconVisibility.reset();
  }, []);
  ...
  function handleInputChange(event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>, valuePassed?:string, key?:string) {
    ...
    if(valuePassed && typeof(valuePassed) === "string" && !formFieldViewListIconVisibility.state[key]){
      value = valuePassed;
    } else{
          ...
          <Grid container rowSpacing={3}>
            {
              Array.from<string>(Object.keys(PhoneNumberFormFieldConfigs)).map((key: string) => {
                const {
                  label, control, required = false, dynamicFieldConditionCheck, isUserAbleToSwitchToInputControl, gridSize = 12
