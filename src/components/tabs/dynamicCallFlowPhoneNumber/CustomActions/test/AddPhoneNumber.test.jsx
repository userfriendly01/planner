...
import { AddPhoneNumber } from "../index";
...
const renderAddFlow = isOpen => {
  return render(
    <AddPhoneNumber openAddModal={openAddModal} newID={1} isOpen={isOpen} duplicateCheck={duplicateCheck} />
  );
};

const renderAddFlowDefaultOpen = () => {
  return render(
    <AddPhoneNumber openAddModal={openAddModal} newID={1} duplicateCheck={duplicateCheck}/>
  );
};

const renderAddFlowCloneType = flowRule => {
  return render(
    <AddPhoneNumber openAddModal={openAddModal} newID={1} cloneType = {true} flowRuleCloned = {flowRule} duplicateCheck={duplicateCheck}/>
  );
};

const renderAddFlowDuplicateCreate = () =>{
  return render(
    <AddPhoneNumber openAddModal={openAddModal} newID={1} isOpen={true} duplicateCheck={duplicateCheckReturnTrue} />);
};
...
describe("<AddFlow />", () => {
  ...
  describe("Add flow Bottom down",()=>{
    test("Validate fields and create flow",()=>{
      ...
      phoneNumberFields[8].valueSetter(validFlowData, { phoneNumberType: "DID1" });
    ...
    test("Validate Flow type with fields",()=>{
      const flowInitRule = phoneNumberFields.reduce((a, v) => ({
        ...a,
        [v.key]: {
          error: false,
