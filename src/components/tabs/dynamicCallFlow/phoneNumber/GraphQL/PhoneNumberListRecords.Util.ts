...
import { dynamicPhoneNumberListRecords } from "./Query/DynamicPhoneNumberListRecords.Query";
import { legacyPhoneNumberListRecords } from "./Query/LegacyPhoneNumberListRecords.Query";
...
export async function listPhoneNumberRecords(accessToken: string): Promise<Array<PhoneNumberRecordType>> {
  ...
  phoneNumberRecords.forEach((callFlowRecord, index) => {
    callFlowRecord.id = index + 1;
  });
