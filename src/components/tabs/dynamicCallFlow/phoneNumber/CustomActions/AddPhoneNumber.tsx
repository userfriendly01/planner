...
import { GREETING_MESSAGES } from "../Form/DynamicPhoneNumberForm.Fields";
...
import { LegacyPhoneNumberFormFieldConfigs } from "../Form/LegacyPhoneNumberFormField.Configs";
...
import {
  FormFieldConfig, FieldDataType, FieldDataTypeEnum, Fields
...
export const AddPhoneNumber = ({
  ...
  function handleInputChange(event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>, valuePassed?:string, key?:string) {
    ...
    formManager.fieldConfigs.state = {
      [key]: {
        ...formManager.fieldConfigs.state[key as keyof Fields],
  ...
  function isFieldConditionMet(key: string): boolean {
    if (LegacyPhoneNumberFormFieldConfigs[key]?.fieldConditionCheck) {
      return LegacyPhoneNumberFormFieldConfigs[key].fieldConditionCheck(formManager.fieldConfigs.state);
    } else {
  ...
  function isValidRecord() {
    ...
    Object.keys(formManager.fieldConfigs.state).forEach((key: string) => {
  ...
  function resetFormFields() {
    formManager.fieldConfigs.reset();
          ...
          <Grid container rowSpacing={3}>
            {
              Array.from<string>(Object.keys(LegacyPhoneNumberFormFieldConfigs)).map((key: string) => {
                ...
                }: FormFieldConfig = LegacyPhoneNumberFormFieldConfigs[key];
