...
import { clearGridMasterData, getGridMasterData } from './GridMaster';
//import AddModal from '../ModalFlow/Add';
//import AdvanceSearchModal from '../ModalFlow/AdvanceSearch/index';
//import EditModal from '../ModalFlow/Edit/index';
import GridColumnDef from './GridColumnDef'
import * as GridColumnDef1 from './GridColumnDef';
import GridSpinner from './GridSpinner';
import {GridStyle} from './GridStyle';
import GridTheme from './GridTheme';
import GridTopHeader from './GridTopHeader';
//import { retrieveFlowData } from '../../services/flowTableService';
import { any } from 'prop-types';

createTheme('gridTheme', { ...GridTheme }, 'gridTheme');
const CACHE_FILTER_FLOW = 'SEARCH_FILTER_FLOW';
const DataGridFlow = (props:any) => {
  ...
  const showToastMessage = (type:String, message:any)=> {
    toast(message, {
      position: 'top-center',
      autoClose: 1500,
   ...
   const loadDataTable = async() => {
    ...
    //let result:any = await retrieveFlowData(accessToken);
    let result:any=[]
    if (result.length > 0) {
      result = result.sort((a:any, b:any) => a.pkey - b.pkey);
      result = result.map((item:any, index:any) => ({ ...item, id: index + 1 }));
  ...
  const filterRecords =()=> {
    const { data, idStart, idEnd } = dataFlow;
