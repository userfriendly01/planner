...
import { RoutingHeadingStyled, RoutingModalBodyStyled, RoutingModalFooterStyled } from "../../AlohaRouting.Styles";
import React, { useState, useRef, useEffect } from "react";
import { convertTime12to24, getAccessToken, getGraphQLEndpoint, initializedAlertBar, routingFields, routingInitRule, routingDropDownList, dayOfWeek } from "utils";
import { CctSharedCallRoutingDb, RoutingDropDownList } from "../../AlohaRouting.Interfaces";
...
import { deleteRoutingRule, updateRoutingDB } from "services";
...
export const EditRouting = ({ isOpen, selectedRow, openEditModal }: EditRoutingComponentProps) => {
    ...
    useEffect(() => {
        ...
        if (selectedRow && selectedRow.startTime) {
            const flagStartTime: boolean = !Date.parse(selectedRow.startTime);
            const flagEndTime: boolean = !Date.parse(selectedRow.endTime);
            const curDate: Date = new Date();
            const todayDate: string = `${curDate.getFullYear()}-${String(curDate.getMonth()).padStart(2, '0')}-${String(curDate.getDate()).padStart(2, '0')}`;
    ...
    const handleInputChange = (
        ...
        if (["startTime", "endTime"].includes(key)) {
            value = handleTimeEvalChange(event, key);
        } else {
