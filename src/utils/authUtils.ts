...
export const getPermissions = (unformattedAdGroups: any): any[] => {
  console.log("*** unformatted AD groups", unformattedAdGroups);
  const myAdGroups = getAdGroups(unformattedAdGroups);
  console.log("*** formatted AD groups", myAdGroups);
  return adGroupPermissionMapping.filter((permission: any) => {
    console.log(`***compare: cn=${permission.adGroup.toLowerCase()}`);
    return myAdGroups.includes(`cn=${permission.adGroup.toLowerCase()}`); });
  // && permission.environment.includes(environment));
};
