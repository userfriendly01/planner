...
export const formatManagersResponse = (response: DbManagerResponse[]): Manager[] => {
  if (response) {
    return response.map<Manager>(manager => {
      console.log("**Type of Manager's Calabrio Teams", typeof Array.from(manager.calabrio_team_ids), Array.from(manager.calabrio_team_ids));
      return {
        ...
        calabrio_team_ids: Array.from(manager.calabrio_team_ids)
      };
    });
  } else {
    return [];
