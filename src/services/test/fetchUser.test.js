...
jest.mock("globals", () => ({
  apiPaths: {
    ...
    EMPLOYEE_LOOKUP_BY_EMAIL: jest.fn().mockReturnValue("/service/employeelookup/email")
  }
}));

describe("msGraph API calls", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    axiosMock.reset();
  });
  ...
  describe("fetchUserByEmail", () => {
    test("call to service succeeds", async () => {
      const fetchUserRes = {
        mail: "email",
        givenName: "FirstName  ",
        surname: "LastName  ",
      ...
      axiosMock.onGet("/service/employeelookup/email").reply(200, fetchUserRes);

      const email = "email";
      const result = await fetchUserByEmail("accessToken", email);
      expect(axiosMock.history.get[0].url).toBe("/service/employeelookup/email");
    ...
    test("should return null when call fails", async () => {
      axiosMock.onGet("/service/employeelookup/email").reply(500, "error");
