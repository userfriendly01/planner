...
describe("deleteUser", () => {
  ...
  describe("service call to DELETE_WORKER succeeds", () => {
    beforeEach(() => {
      axiosMock.onPost(`${env.ADMIN_CLIENT_URL}/termination`).reply(200, { wow: "Yay!" });
    });
    test("should resolve with string", done => {
      ...
      terminateUser(accessToken, workerPayload).then(resolvedVal => {
        expect(axiosMock.history.post[0].url).toBe(`${env.ADMIN_CLIENT_URL}/termination`);
  ...
  describe("service call to DELETE_WORKER fails", () => {
    ...
    beforeEach(() => {
      axiosMock.onPost(`${env.ADMIN_CLIENT_URL}/termination`).reply(status, badResponse);
    });
