...
describe("deleteUser", () => {
  ...
  describe("service call to DELETE_WORKER succeeds", () => {
    beforeEach(() => {
      axiosMock.onPost("undefinedtermination").reply(200, { wow: "Yay!" });
    });
    test("should resolve with string", done => {
      ...
      terminateUser(accessToken, workerPayload).then(resolvedVal => {
        expect(axiosMock.history.post[0].url).toBe("undefinedtermination");
  ...
  describe("service call to DELETE_WORKER fails", () => {
    ...
    beforeEach(() => {
      axiosMock.onPost("undefinedtermination").reply(status, badResponse);
    });
